...
**v1.2.x 系列**：修正 SPA observer rescan 無限迴圈（v1.2.0——Wikipedia 等頁面翻譯完成後 toast「已翻譯 N 段新內容」不斷重複出現。根因：fragment 類型的翻譯單位注入後，父元素不帶 `data-shinkansen-translated` 屬性（因為父元素含 block 後代，不能標記整個元素為「已翻譯」），導致 `collectParagraphs` 的 `extractInlineFragments` 在 rescan 時重新收集已翻成繁中的 fragment——CJK 字元 `\u3400-\u9fff` 通過原有 regex 檢查。`SPA_OBSERVER_MAX_RESCANS = Infinity` 無上限，每次 rescan 翻譯注入又觸發 MutationObserver，形成無限迴圈。修法：`extractInlineFragments` 的 `flushRun()` 新增 `isTraditionalChinese` 過濾——已翻成繁中的 inline run 不再被收集為翻譯單位，與 `isCandidateText` 對 element 單位的邏輯一致。此為結構性通則：任何語言的翻譯結果若為繁中都會被正確過濾，不依賴站點特判）。
